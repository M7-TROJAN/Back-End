...
namespace LINQTut15.Except
{
    internal class Program
    {
        static void Main(string[] args)
        {
            ...
            var set3 = set1.Except(set2); // set1 - set2 => emplyees that are in set1 but not in set2 based on reference equality
            set3.Print($"========= set1.Except(set2) Participants ({set3.Count()})");

            var set4 = set1.ExceptBy(set2.Select(x => x.EmployeeNo),  x => x.EmployeeNo); // set1 - set2 => emplyees that are in set1 but not in set2 based on EmployeeNo
